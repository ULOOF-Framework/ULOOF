...
public class PackManager {
    ...
    private DexPrinter[] dexPrinterArray = null;
    
    private boolean multiDexing = false; //multidexing disabled as default
    private int numDexes = 1;
    ...
    public void setMultiDexing(boolean bool) { //use to set multidexing
    	multiDexing = bool;
    }
    public void setNumDexes(int numDexes) { //use to set multidexing
    	this.numDexes = numDexes;
    }
	...
	public void writeOutput() {
		...
		if (Options.v().output_format() == Options.output_format_dava) {
			...
			writeDexOutput(new ArrayList<String>(),""); //here arguments no matters
		} else {
	...
	public void writeOutput(ArrayList<String> mainDexClasses, String apkName) {
		setupJAR();
		if (Options.v().verbose())
			PhaseDumper.v().dumpBefore("output");
		if (Options.v().output_format() == Options.output_format_dex
				|| Options.v().output_format() == Options.output_format_force_dex) {
			writeDexOutput(mainDexClasses,apkName);
			}
	...
	protected void writeDexOutput(ArrayList<String> mainDexClasses, String apkName)  {
		//int numDexes = Math.ceil((double)methodsNumber/65536.0);
		if(this.numDexes == 1){
			System.out.println("WriteDexOutoutOneDex");
			try {
				System.in.read();
			} catch (IOException e) {
			...
			System.out.println("WriteDexOutMultipleDex");
			dexPrinterArray = new DexPrinter[50]; //we initialize 50 dex guessing that is unlucky to have more than 50 dexes
			for(int i=0;i<50;i++) {
				dexPrinterArray[i]=new DexPrinter();
			}
			ArrayList<Chain<SootClass>> itrArray = new ArrayList<Chain<SootClass>>(); //itrArry(i) memorize the i'th dex's classes
			Chain<SootClass> allClasses = Scene.v().getApplicationClasses();
			Iterator<SootClass> itrAll;
			itrArray.add(0,new HashChain<SootClass>());
			Iterator<String> itr = mainDexClasses.iterator();
			while(itr.hasNext()) { //searches through all classes those to be placed in the first dex
				String current = itr.next();
				SootClass currentClass = Scene.v().getSootClassUnsafe(current);
				if( currentClass !=null) {
					allClasses.remove(currentClass);
					if(!itrArray.get(0).contains(currentClass)) {
						itrArray.get(0).add(currentClass);
						}
					}
				
			}
		//	System.out.println("size= "+itrArray.get(0).size());
			//int numMet=0;
			Iterator<SootClass> iterat = itrArray.get(0).iterator();
		/*	while(iterat.hasNext()) {
				SootClass cl=  iterat.next();
				numMet+=cl.getMethodCount();
			}
			System.out.println(numMet);
		*/
			itrAll = allClasses.iterator();
			int count = 0, index=0;
			//itrArray.add(1,new HashChain<SootClass>());
			while(itrAll.hasNext()) { // now fulfill the list of classes for the other dexes needed
				SootClass current = itrAll.next();
				int methodNum = current.getMethodCount();
				count+=methodNum;
				if(count-(40000*(index+1)) >= 0) { //conservative index because soot adds some methods while writing .classes
					index++;
					itrArray.add(index,new HashChain<SootClass>());
					//System.out.println("list 0 size= "+itrArray.get(0).size());
					//System.out.println("list "+index+" size= "+itrArray.get(index).size());
				}
				itrArray.get(index).add(current);
			}
			/*for(int i=0;i<index+1;i++) {
				System.out.println("Size of the "+i+" is= "+itrArray.get(i).size());
			}*/
			/*Now triggers the writing of all dexes*/
			System.out.println("Now writing the needed dexes");
			for(int i=0;i<(index+1);i++){
				writeOutput(itrArray.get(i).iterator(),dexPrinterArray[i]);
				dexPrinterArray[i].print(i,apkName);
				dexPrinterArray[i]= null;
			}
			
			/*Finally add the dexes inside the apk*/
			...
			for(String str:children) {
				fileList.add(new File(outputDir+"/"+str));
			}
			
			ApkHandler apkH;
			try {
//				String[] splittedString = apkName.split("/");
				String[] splittedString = apkName.split(Pattern.quote(File.separator));
				String apkToBeHandled = splittedString[splittedString.length-1];
//				apkH = new ApkHandler(new File(outputDir+"/"+apkToBeHandled));
				apkH = new ApkHandler(new File(outputDir+File.separator+apkToBeHandled));
				apkH.addFilesToApk(fileList);
			} catch (IOException e) {
				e.printStackTrace();
			}
			
			/*Now clears the output directory from .dex puttent inside the apk*/
			
			File[] dexToDelete = dir.listFiles(filter);
			for(File f:dexToDelete) {
				f.delete();
			}
	...
	private void writeOutput(Iterator<SootClass> classes,DexPrinter dexPrinterA) {
	...
	private void writeClass(SootClass c, DexPrinter dexPrinterA) {
